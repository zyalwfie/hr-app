<x-dashboard-layout>
    ...
    <x-slot:title>
        Dashboard | Leave Request - Edit, {{ $leave_request->employee->fullname }}
    </x-slot:title>
        ...
        <div class="shadow-xs rounded-xl border border-gray-200 bg-white p-4 sm:p-7 dark:bg-neutral-800">
            <div class="mb-8">
                ...
                <p class="text-sm text-gray-600 dark:text-neutral-400">
                    Make changes employee leave request.
                </p>
            </div>

            <form action="{{ route('leave-requests.update', $leave_request->id) }}" method="post">
                @csrf
                @method('put')
                ...
                <div class="grid gap-2 sm:grid-cols-12 sm:gap-6">
                            ...
                            <select name="employee_id" id="employee"
                                ...
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}"
                                        {{ old('employee_id', $leave_request->employee->id) == $employee->id ? 'selected' : '' }}>
                    ...
                    <div class="sm:col-span-9">
                        <div class="relative">
                            <input id="range_date" type="text" name="range_date" value="{{ old('range_date') }}"
                                data-start-date="{{ old('range_date') ? null : $leave_request->start_date }}"
                                data-end-date="{{ old('range_date') ? null : $leave_request->end_date }}"
                    ...
                    <div class="sm:col-span-3">
                        <label for="leave_type" class="mt-2.5 inline-block text-sm text-gray-800 dark:text-neutral-200">
                            Leave Type
                        </label>
                    </div>
                    <!-- End Col -->
                        ...
                        <div class="relative">
                            <select name="leave_type" id="leave_type"
                                ...
                                <option value="sick leave"
                                    {{ old('leave_type', $leave_request->leave_type) == 'sick leave' ? 'selected' : '' }}>
                                    Sick Leave</option>
                                <option value="maternity leave"
                                    {{ old('leave_type', $leave_request->leave_type) == 'maternity leave' ? 'selected' : '' }}>
                                    Maternity Leave</option>
                                <option value="vacation"
                                    {{ old('leave_type', $leave_request->leave_type) == 'vacation' ? 'selected' : '' }}>
                                    Vacation</option>
                            </select>
                            @error('leave_type')
                                <div class="pointer-events-none absolute inset-y-0 end-0 flex items-center pe-10">
                ...
                <div class="mt-5 flex justify-end gap-x-2">
                    <a href="{{ route('leave-requests.show', $leave_request->id) }}"
